...
public class Drugs implements IDrugs {
	...
	public DrugInteraction getDrugInteraction(List<String> drugNames, String primaryDrugName) {
		RestTemplate restTemplate = new RestTemplate();
		UriComponentsBuilder builder = UriComponentsBuilder.fromHttpUrl(DRUG_INTERACTION_URL).queryParam("drugname",
				drugNames.toArray(new String[0]));
		DrugInteraction drugInteration = restTemplate.getForObject(builder.toUriString(), DrugInteraction.class);
		List<Datum> rows = drugInteration.getData();
		Iterator<Datum> rowIterator = rows.iterator();
		while (rowIterator.hasNext()) {
			Datum row = rowIterator.next();
			boolean primaryDrugFound = false;
			for (String name : row.getInteractionSetADrugList()) {
				if (primaryDrugName.equalsIgnoreCase(name)) {
					primaryDrugFound = true;
					break;
				}
			}
			if (!primaryDrugFound) {
				for (String name : row.getInteractionSetBDrugList()) {
					if (primaryDrugName.equalsIgnoreCase(name)) {
						primaryDrugFound = true;
						break;
					}
	...
	public TradeName getDrugTradeName(String drugName) {
		...
		// TradeName
		// tradeName=restTemplate.getForObject(DRUG_FROM_TRADE_URI+drugName,
		// TradeName.class);
